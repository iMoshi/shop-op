...
const CartNotification = ({ product, modalShow, modalOnHide }) => {
  ...
  const { color, name, website_image, variations, quantity } = product;
    ...
    <Modal show={modalShow} onHide={modalOnHide} id="modalAddToCartProduct">
      ...
      <ModalBody>
        <div className="tt-modal-addtocart mobile">
          ...
          <Link href="/">
            <a className="btn-link btn-close-popup">CONTINUE SHOPPING</a>
          </Link>
          <Link href="/cart">
            <a className="btn-link">VIEW CART</a>
          </Link>
          <Link href="/checkout">
            <a className="btn-link">PROCEED TO CHECKOUT</a>
          </Link>
        </div>
                    ...
                    variations.map(
                      (variation, i) =>
                        variation.color.name === color && (
                          <img key={i} src={variation.color.thumb} alt={name} />
                        )
                    )
                  ) : (
                    <img
                      src={`https://meta.omnismart.mn/${website_image}`}
                      alt={name}
                    />
                  )}
                </div>
              ...
              <div className="tt-product-total">
                <div className="tt-total">
                  ...
                  <span className="tt-price ml-1">
                    {CURRENCY +
                      getCartProductTotalPrice(shoppingCart, product).toFixed(
                        2
                      )}
                  </span>
                </div>
            ...
            <Col lg={6}>
              <Link href="/">
                <a className="tt-cart-total">
                  There are {shoppingCart && shoppingCart.length} items in your
                  cart
                  <div className="tt-total">
                    TOTAL:{" "}
              ...
              <Link href="/shop">
                <a className="btn btn-border btn-close-popup">
                  CONTINUE SHOPPING
                </a>
              </Link>
              <Link href="/cart">
                <a className="btn btn-border">VIEW CART</a>
              </Link>
              <Link href="/checkout">
