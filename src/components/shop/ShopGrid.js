...
const ShopGrid = ({
  ...
  const [layout, setLayout] = useState("tt-col-four");
  const { allProducts, filterPanelShow, filterPanelHandler, getFilterParam } =
    useFilter(products?.newProductItem);
  const { onSortHandler, sortProducts } = useSort(allProducts);
  const [filteredProd, setFiltered] = useState(allProducts);

  const onBrandHandler = (brand) => {
    console.log(allProducts);

    const filteredProduct = allProducts.filter((e) => brand == e.brand);
    console.log(filteredProduct);
    setFiltered(filteredProduct);
    console.log(filteredProd);
  };

  const onCatHandler = (cat) => {
    console.log(cat);
    console.log(filteredProd);
    const filteredProduct = allProducts.filter((e) => cat == e.item_group);
    console.log(filteredProduct);
    setFiltered(filteredProduct);
  };
  const getLayout = (col) => {
          ...
          <Col xs={12} lg={sidebar ? 9 : 12}>
            <div className="content-indent container-fluid-custom-mobile-padding-02">
              <ShopOptions
                ...
                productLength={filteredProd.length}
                filterPanelHandler={filterPanelHandler}
                onProductPerPageHandler={onProductPerPageHandler}
              />
              <ShopProducts
