...
const ShopFilters = (props) => {
  ...
  const cat = [
    { name: "Ухаалаг утас", value: "Mobile phone" },
    { name: "Ухаалаг цаг", value: "Ухаалаг цаг" },
    { name: "Дагалдах хэрэгсэл", value: "Дагалдах хэрэгсэл" },
    { name: "Чихэвч", value: "Ухаалаг чихэвч" },
  ];
  ...
  return (
    <Fragment>
      ...
      <ShopWidget title="Ангилал">
        <ul className="tt-list-row">
          {cat.slice(0, categoryLimit).map((cat) => (
            <li
              key={cat.value}
              className={cat.value === slug ? "active" : ""}
              onClick={() => onCatHandler(cat.value)}
            >
              {/* <Link href={`/shop/category/${cat}`}>
                {toCapitalize(cat.replace(/-/g, " "))}
              </Link> */}
              {cat.name}
            </li>
          ))}
      ...
      <ShopWidget title="Брэндээр шүүх">
        <ul className="tt-list-row">
          {/* {sizes.map((size) => (
          ...
          {brand.slice(0, categoryLimit).map((cat) => (
            <li
              onClick={() => onBrandHandler(cat)}
              key={cat}
              className={cat === slug ? "" : "active"}
            >
              {/* <Link href={`/product/category/${cat}`}>
                {toCapitalize(cat.replace(/-/g, " "))}
              </Link> */}
              <a>{cat.toUpperCase()}</a>
            </li>
