...
export const getProducts = (products, type, limit) => {
  ...
  if (type === "featured" || type === "tending" || type === "specials") {
    ...
    const filteredProducts = nonFilteredProducts.sort(
    ...
    const filteredProducts = nonFilteredProducts.filter(
      (product) =>
        product.discount !== null || false || undefined || product.discount > 0
    );
    ...
    return nonFilteredProducts.slice(
      0,
      limit < 0
        ? nonFilteredProducts.length
        : limit
        ? limit
        : nonFilteredProducts.length
    );
...
export const getProductsUniqueCategories = (products, limit) => {
  const categories = products.map((product) => product.vendor);
...
export const getSpecificCategories = (products, cats = []) => {
  ...
  const categ = ["Apple", "Samsung"];

  return categ.filter((field) => categories.find((cat) => cat === field));
};
...
export const getProductsUniqueColors = (products) => {
  ...
  flatDeep(colors).forEach((el) => {
    const isInArr =
      el && uniqueColors.findIndex((item) => el?.name === item?.name);
    if (isInArr < 0) {
...
export const getProductUniqueMaterials = (product) => {
  const { variations } = product;
  const lists =
    variations && variations.map((variation) => variation.materials);
  const materials =
    variations && flatDeep(lists).map((material) => material.name);
...
export const getProductsUniqueTags = (products, limit) => {
  ...
  return filteredTags.slice(
    ...
    limit < 0 ? filteredTags.length : limit ? limit : filteredTags.length
  );
};
...
export const getProductUniquePrices = (product) => {
  ...
  const prices = variations
    ? flatDeep(lists).map((material) =>
        product.discount
          ? getDiscountPrice(material.price, product.discount)
          : material.price
      )
    : [
        product.discount
