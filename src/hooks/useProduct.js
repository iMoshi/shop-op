...
const useProduct = (product) => {
  const { name, discount, variations, item_name } = product;
  ...
  const slug = `/products/${item_name.toLowerCase().split(" ").join("-")}`;
  ...
  const wishlistHandler = (event) => {
    ...
    !isInWishlist
      ? cogoToast.success(`"${name}" is successfully added.`, {
  ...
  const productQuantityIncrement = () =>
    setProductQuantity((prevState) =>
      prevState < productStock - cartProductQuantity
        ? (prevState += 1)
        : prevState
    );
  ...
  useEffect(() => {
    ...
    setProductPrice(
      variations ? variations[0]?.materials[0]?.price : product.price
    );
  }, []);
